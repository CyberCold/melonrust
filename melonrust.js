...
//[модалка китов]
<button onclick="showKits()">Посмотреть содержимое китов</button>

<div id="kitsModal" style="display:none;">
    <div id="kitsContent"></div>
    <button onclick="closeModal()">Закрыть</button>
</div>

<script>
    async function showKits() {
        try {
            const response = await fetch('/assets/json/KitsList.json'); // путь поправь под свой
            const data = await response.json();

            // допустим, у тебя есть группа "vip"
            const group = "vip";
            const kits = Object.entries(data).filter(([key]) => key.startsWith(group));

            let html = '';
            for (const [kitName, kitItems] of kits) {
                html += `<h3>${kitName}</h3><ul>`;
            ...
            document.getElementById("kitsContent").innerHTML = html;
            document.getElementById("kitsModal").style.display = "block";
        } catch (err) {
            console.error("Ошибка при загрузке китов:", err);
        }
    }

    function closeModal() {
        document.getElementById("kitsModal").style.display = "none";
    }
</script>
